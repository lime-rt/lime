...
popsin(configInfo *par, struct grid **gp, molData **md, int *popsdone){
  ...
  par->numDensities = 1;
  assert(fread(&par->radius,   sizeof(double), 1, fp)==1);
  assert(fread(&par->ncell,    sizeof(int), 1, fp)==1);
  assert(fread(&par->nSpecies, sizeof(int), 1, fp)==1);
  ...
  *md=realloc(*md, sizeof(molData)*par->nSpecies);
  ...
  for(i=0;i<par->nSpecies;i++){
    ...
    (*md)[i].cmb = NULL;
    (*md)[i].gir = NULL;
    assert(fread(&(*md)[i].nlev,  sizeof(int),        1,fp)==1);
    assert(fread(&(*md)[i].nline, sizeof(int),        1,fp)==1);
    assert(fread(&(*md)[i].npart, sizeof(int),        1,fp)==1);
    (*md)[i].part = malloc(sizeof(*((*md)[i].part))*(*md)[i].npart);
    for(j=0;j<(*md)[i].npart;j++){
      setCollPartsDefaults(&((*md)[i].part[j]));
      assert(fread(&(*md)[i].part[j].ntrans, sizeof(int), 1,fp)==1);
    }
    (*md)[i].lal=malloc(sizeof(int)*(*md)[i].nline);
    for(j=0;j<(*md)[i].nline;j++) assert(fread(&(*md)[i].lal[j],    sizeof(int), 1,fp)==1);
    (*md)[i].lau=malloc(sizeof(int)*(*md)[i].nline);
    for(j=0;j<(*md)[i].nline;j++) assert(fread(&(*md)[i].lau[j],    sizeof(int), 1,fp)==1);
    (*md)[i].aeinst=malloc(sizeof(double)*(*md)[i].nline);
    for(j=0;j<(*md)[i].nline;j++) assert(fread(&(*md)[i].aeinst[j], sizeof(double), 1,fp)==1);
    (*md)[i].freq=malloc(sizeof(double)*(*md)[i].nline);
    for(j=0;j<(*md)[i].nline;j++) assert(fread(&(*md)[i].freq[j],   sizeof(double), 1,fp)==1);
    (*md)[i].beinstl=malloc(sizeof(double)*(*md)[i].nline);
    for(j=0;j<(*md)[i].nline;j++) assert(fread(&(*md)[i].beinstl[j],sizeof(double), 1,fp)==1);
    (*md)[i].beinstu=malloc(sizeof(double)*(*md)[i].nline);
    for(j=0;j<(*md)[i].nline;j++) assert(fread(&(*md)[i].beinstu[j],sizeof(double), 1,fp)==1);
    for(j=0;j<(*md)[i].nline;j++) assert(fread(&dummy,sizeof(double), 1,fp)==1);
    assert(fread(&dummy, sizeof(double),      1,fp)==1);
    assert(fread(&dummy, sizeof(double),      1,fp)==1);
  }
  ...
  for(i=0;i<par->ncell;i++){
    assert(fread(&(*gp)[i].id, sizeof (*gp)[i].id, 1, fp)==1);
    assert(fread(&(*gp)[i].x, sizeof (*gp)[i].x, 1, fp)==1);
    assert(fread(&(*gp)[i].vel, sizeof (*gp)[i].vel, 1, fp)==1);
    assert(fread(&(*gp)[i].sink, sizeof (*gp)[i].sink, 1, fp)==1);
    for(j=0;j<par->nSpecies;j++)
      assert(fread(&(*gp)[i].mol[j].nmol, sizeof(double), 1, fp)==1);
    assert(fread(&(*gp)[i].dopb_turb, sizeof (*gp)[i].dopb_turb, 1, fp)==1);
    for(j=0;j<par->nSpecies;j++){
      (*gp)[i].mol[j].pops=malloc(sizeof(double)*(*md)[j].nlev);
      for(k=0;k<(*md)[j].nlev;k++) assert(fread(&(*gp)[i].mol[j].pops[k], sizeof(double), 1, fp)==1);
      for(k=0;k<(*md)[j].nline;k++) assert(fread(&dummy, sizeof(double), 1, fp)==1); /* knu */
      for(k=0;k<(*md)[j].nline;k++) assert(fread(&dummy, sizeof(double), 1, fp)==1); /* dust */
      assert(fread(&(*gp)[i].mol[j].dopb,sizeof(double), 1, fp)==1);
      assert(fread(&(*gp)[i].mol[j].binv,sizeof(double), 1, fp)==1);
    }
    assert(fread(&dummy, sizeof(double), 1, fp)==1);
    assert(fread(&dummy, sizeof(double), 1, fp)==1);
    assert(fread(&dummy, sizeof(double), 1, fp)==1);
