...
void old_raytrace(int im, inputPars *par, struct grid *g, molData *m, image *img){
	...
	int ichan, posn, oposn,i,px,iline,tmptrans;
...
*/
		...
		if(sqrt(xp*xp+yp*yp)<=par->radius) {
			...
			if(!par->polarization){
			  for(ichan=0;ichan<img[im].nchan;ichan++){
				img[im].pixel[px].intense[ichan]+=subintens[ichan];
				img[im].pixel[px].tau[ichan]+=tau[ichan];
			  }
			} else {
			  for(ichan=0;ichan<3;ichan++){
			    img[im].pixel[px].intense[ichan]+=subintens[ichan];
			  }
			  img[im].pixel[px].intense[3]+=sqrt(pow(img[im].pixel[px].intense[1],2.)+pow(img[im].pixel[px].intense[2],2.))/subintens[0];
			}
		}	
